...
export class HeroeService {
    ...
    public getHomeHero(idx: number): string {
        this.heroe = this.heroes[idx];
    ...
    public buscarHeroe(termino: string): Heroe[] {
        ...
        for (let i = 0; i < this.heroes.length; i++) {
          let item: Heroe = this.heroes[i];
          if (item.nombre.toLocaleLowerCase().indexOf(termino.toLocaleLowerCase()) >= 0) {
              item.idx = i;
              listadoHeroes.push(item);
          }
        }
